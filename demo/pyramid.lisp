    ...
    (let ((friction 0.6)
      ...
      (loop with n = 9
            ...
            do (loop for j from 0 below i
                     do (mapc (lambda (body)
                                (attach-shape (make-poly verts :friction friction) body)
                                (world-add-body world body))
                              (nconc (list (make-body :mass 1
                                                      :position (vec+ (vec (* j 60)
                                                                           -220)
                                                                      offset))
                                           (make-body :mass 1
                                                      :position (vec+ (vec (* j 60)
