...
(defun make-leg (world side offset walker anchor &aux (leg-mass 1))
  (let* ((upper-leg (world-add-body
    ...
    (push upper-leg (walker-upper-legs walker))
    (push lower-leg (walker-lower-legs walker))
    ;; add constraints
    (world-add-constraint world
                          (make-pivot-joint chassis upper-leg (vec offset 0) +zero-vector+))
    (world-add-constraint world
                          (make-pin-joint chassis lower-leg (vec offset 0) +zero-vector+))
    (world-add-constraint world
                          (make-gear-joint upper-leg lower-leg 0 1))
...
(defmethod init-demo ((demo theo-jansen))
  (let ((world (make-world :iterations 20 :gravity (vec 0 -500))))
    (setf (static-body demo)
          (world-add-body world
                          (make-body :actor :not-grabbable
                                     :shapes (list (make-segment (vec -320 -240)
                                                                 (vec -320 240)
                                                                 :restitution 1
                                                                 :friction 1)
                                                   (make-segment (vec 320 -240)
                                                                 (vec 320 240)
                                                                 :restitution 1
                                                                 :friction 1)
                                                   (make-segment (vec -320 -240)
    ...
    (let ((walker (make-instance 'walker))
          ...
          (side 30)
          (num-legs 2))
      (setf (demo-walker demo) walker)
      (setf (walker-chassis walker)
            (world-add-body world
                            (make-body :mass chassis-mass
                                       :actor walker
                                       :shapes (list (make-segment (vec (- offset) 0)
                                                                   (vec offset 0)
                                                                   :radius *seg-radius*)))))
      (setf (walker-crank walker)
            (world-add-body world
                            (make-body :mass crank-mass
                                       :actor walker
                                       :shapes (list (make-circle crank-radius)))))
      (world-add-constraint world
                            (make-pivot-joint (walker-chassis walker)
                                              (walker-crank walker)
                                              +zero-vector+
                                              +zero-vector+))
      ;; add the legs...
      ...
      (setf (demo-motor demo)
            (world-add-constraint world
                                  (make-simple-motor (walker-chassis walker)
                                                     (walker-crank walker)
                                                     6)))
      world)))
